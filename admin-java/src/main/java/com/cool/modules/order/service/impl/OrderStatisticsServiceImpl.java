...
public class OrderStatisticsServiceImpl extends
    ...
    @Override
    public int statistics(OrderStatisticsDto dto) {
    ...
    @Override
    public int statisticsThisWeek() {
      // 获取本周一和周日
      Date startDate = DateUtil.beginOfWeek(DateUtil.date());
      Date endDate = DateUtil.endOfWeek(DateUtil.date());

      // 如果已经统计过，则不进行统计
      OrderStatisticsEntity orderStatisticsEntity = mapper.selectOneByQuery(QueryWrapper.create(new OrderStatisticsEntity())
        .eq(OrderStatisticsEntity::getStartDate, startDate)
        .eq(OrderStatisticsEntity::getEndDate, endDate));
