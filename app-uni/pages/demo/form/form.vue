...
<template>
  <cl-page :padding="20">
    <view class="page">
      <cl-card label="基础用法">
        <cl-form ref="Form" v-model="form" :rules="rules" :disabled="loading" label-position="left">
          <cl-form-item label="活动名称" prop="name">
            <cl-input v-model="form.name" placeholder="请填写活动名称" />
          </cl-form-item>

          <cl-form-item label="活动时间" prop="date">
            <cl-select-date v-model="form.date" />
          </cl-form-item>

          <cl-form-item label="活动类型" prop="type">
            <cl-select v-model="form.type" :options="options.type" />
          </cl-form-item>

          <cl-form-item label="活动人数" prop="num" justify="end">
            <cl-input-number v-model="form.num" :min="1" :max="100" />
          </cl-form-item>

          <cl-form-item label="活动区域" prop="area" label-position="top">
            <cl-checkbox-group v-model="form.area">
              <cl-checkbox v-for="(item, index) in options.area" :key="index" :label="item.value">
                {{ item.label }}
              </cl-checkbox>
            </cl-checkbox-group>
          </cl-form-item>
          ...
          <cl-form-item label="资源" prop="source" label-position="top">
            <cl-radio-group v-model="form.source">
              <cl-radio v-for="(item, index) in options.source" :key="index" :label="item.value">
                {{ item.label }}
              </cl-radio>
            </cl-radio-group>
          </cl-form-item>
          ...
          <cl-form-item label="活动赞助商" prop="company" label-position="top">
            ...
            <template #append>
              <cl-icon name="help-border" :size="36" />
            </template>
          </cl-form-item>
        </cl-form>
      </cl-card>
      ...
      <cl-footer border>
        <cl-button size="large" round fill @tap="clear">
          清空
        </cl-button>
        <cl-button size="large" round fill type="success" :loading="loading" @tap="submit">
          提交
        </cl-button>
      </cl-footer>
    </view>
